...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showConfirmModal(callback, message = null) {
        document.getElementById('confirm-text').textContent = message || translations[currentLang]['confirmDelete'];
    ...
    async function checkAuthStatus() {
        try {
            ...
            try {
                // If the error was from /api/auth/verify returning 401/403, we should clear the token
                // But only if we are sure it's an invalid token, not a network error.
                // Assuming apiFetch throws or returns non-ok for 401.
    ...
    function displayVideos(videos) {
        ...
        videos.forEach(video => {
            ...
            // Use textContent for description to prevent XSS if it was set via innerHTML previously
        ...
        try {
            ...
            try {
                ...
                if (r.ok) {
                    ...
                    cl.innerHTML = (rows||[]).map(c=>`<div class=\"p-3 bg-gray-800 rounded\"><div class=\"text-sm font-semibold\">${escapeHtml(c.username||'Guest')}</div><div class=\"text-sm text-gray-300\">${escapeHtml(c.text)}</div><div class=\"text-xs text-gray-400\">${new Date(c.created_at).toLocaleString()}</div></div>`).join('')
            ...
            try {
                const r = await apiFetch('/api/history'); if (r.ok) { const hist = await r.json(); const cont = document.getElementById('continue-list-home'); if (cont) cont.innerHTML = (hist||[]).slice(0,5).map(h=>`<a href=\"/video.html?id=${h.video_id}\" class=\"block p-2 bg-gray-800 rounded\">${escapeHtml(h.title||'Video')}</a>`).join(''); }
            } catch(e){}
    ...
    function renderSmallList(container, videos) {
        ...
        container.innerHTML = videos.map(v => {
            ...
            <a href="/video.html?id=${v.id}" class="flex items-center gap-3 p-2 bg-gray-800 rounded hover:bg-gray-700">
                ...
                <div class="flex-1">
                    <div class="font-semibold">${escapeHtml(v.title)}</div>
                    <div class="text-xs text-gray-400">${escapeHtml(v.category || '')}</div>
                </div>
            </a>
        `}).join('');
    }
    ...
    async function populateHomepageExtras() {
        try {
            ...
            if (fRes.ok) {
                ...
                if (featured.length) {
                    const v = featured[0];
                    featuredWrap.innerHTML = `<a href="/video.html?id=${v.id}" class="block rounded overflow-hidden"><div class="aspect-video bg-black"><img src='${v.thumbnail_url || ''}' class='w-full h-full object-cover' /></div><div class='p-3'><h3 class='text-lg font-bold'>${escapeHtml(v.title)}</h3><p class='text-gray-400'>${escapeHtml(v.description||'')}</p></div></a>`;
            ...
            items.forEach(it => {
                ...
                pc.innerHTML = `
                    <div class="popular-hero">
                        <div class="hero-card cursor-pointer" data-cat="${escapeHtml(hero.name)}">
                            ${hero.thumb ? `<img src="${hero.thumb}" alt="${escapeHtml(hero.name)}" />` : ''}
                            <div class="hero-overlay"></div>
                            <div class="hero-content"><div class="hero-title">${escapeHtml(hero.name)}</div><div class="hero-sub">${(hero.count||0)} videos</div></div>
                        </div>
                        <div class="side-cards">
                            ${side.map(si=>`<div class="popular-chip cursor-pointer" data-cat="${escapeHtml(si.name)}"><img src="${si.thumb||('https://placehold.co/120x80/0b1020/9ca3af?text='+encodeURIComponent(si.name))}"/><div class="meta"><div class="name">${escapeHtml(si.name)}</div><div class="count">${(si.count||0)} videos</div></div></div>`).join('')}
                        </div>
                    </div>
                `;
        ...
        try {
            ...
            if (r.ok) {
                ...
                cont.innerHTML = (hist||[]).slice(0,5).map(h=>`<a href="/video.html?id=${h.video_id}" class="block p-2 bg-gray-800 rounded">${escapeHtml(h.title||'Video')}</a>`).join('');
    ...
    videoForm.addEventListener('submit', async (e) => {
        ...
        try {
            if (file) {
                ...
                try {
                    await uploadVideoFile(formData, (pct) => { if (bar) bar.style.width = pct + '%'; if (text) text.textContent = pct + '%'; }, id);
                    hideModal(videoModal);
                    showToast(id ? translations[currentLang]['videoUpdated'] : translations[currentLang]['videoAdded'], 3500, 'success');
                    fetchVideos();
                } catch (err) {
    ...
    function renderUsers(users) {
        ...
        manageUsersList.innerHTML = users.map(u => `
            <div class="flex justify-between items-center bg-gray-700 p-2 rounded-md user-row" data-username="${escapeHtml(u.username)}" data-role="${escapeHtml(u.role || 'user')}" data-suspended="${u.suspended ? 1 : 0}">
                <div>
                    <div class="font-medium text-white">${escapeHtml(u.username)} ${u.suspended ? '<span class="text-xs text-yellow-300 ml-2">(Suspended)</span>' : ''}</div>
                    <div class="text-xs text-gray-400">${escapeHtml(u.role || 'user')}</div>
                </div>
                <div class="flex items-center space-x-2">
                    <button class="btn-edit-user text-sm py-1 px-2 rounded bg-indigo-600 text-white" data-username="${escapeHtml(u.username)}">Edit</button>
                    <button class="btn-activity-user text-sm py-1 px-2 rounded bg-gray-600 text-white" data-username="${escapeHtml(u.username)}" data-id="${u.id}">Activity</button>
                    <button class="btn-suspend-user text-sm py-1 px-2 rounded ${u.suspended ? 'bg-green-600' : 'bg-yellow-600'} text-white" data-username="${escapeHtml(u.username)}">${u.suspended ? 'Un-suspend' : 'Suspend'}</button>
                    <button class="btn-delete-user text-sm py-1 px-2 rounded bg-red-600 text-white" data-username="${escapeHtml(u.username)}">Delete</button>
                </div>
            </div>
        `).join('');
    }
    ...
    function uploadVideoFile(formData, onProgress, videoId) {
        return new Promise((resolve, reject) => {
            const xhr = new XMLHttpRequest();
            const targetUrl = videoId ? `/api/videos/${videoId}` : '/api/videos';
            ...
            xhr.onreadystatechange = () => {
                if (xhr.readyState === 4) {
                    if (xhr.status >= 200 && xhr.status < 300) {
                        ...
                        const err = { status: xhr.status, text: xhr.responseText };
                        reject(err);
                    }
                }
            };
