...
(async function(){
    ...
    // comments
        async function loadComments(){
            ...
            commentList.innerHTML = rows.map(c=>`<div class="p-3 bg-gray-800 rounded"><div class="text-sm font-semibold">${escapeHtml(c.username||'Guest')}</div><div class="text-sm text-gray-300">${escapeHtml(c.text)}</div><div class="text-xs text-gray-400">${new Date(c.created_at).toLocaleString()}</div></div>`).join('');
        }
        await loadComments();
        ...
        if (histRes.ok) {
            ...
            const cont = hist.filter(h=>h.video_id && String(h.video_id)!==String(id)).slice(0,5).map(h=>`<a href="/video.html?id=${h.video_id}" class="block p-2 bg-gray-800 rounded">${escapeHtml(h.title||'Video')}</a>`).join('');
            continueList.innerHTML = cont;
        }

        // related: simple same-category
        const related = videos.filter(v=>v.category===vid.category && String(v.id)!==String(id)).slice(0,5).map(v=>`<a href="/video.html?id=${v.id}" class="block p-2 bg-gray-800 rounded">${escapeHtml(v.title)}</a>`).join('');
